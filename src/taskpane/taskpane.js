/*
 * Copyright (c) Microsoft Corporation. All rights reserved. Licensed under the MIT license.
 * See LICENSE in the project root for license information.
 */

/* global console, document, Excel, Office */
import fetch from 'node-fetch';
//import fetch from 'cross-fetch';

Office.onReady(info => {
  if (info.host === Office.HostType.Excel) {
    document.getElementById("sideload-msg").style.display = "none";
    document.getElementById("app-body").style.display = "flex";
    document.getElementById("run").onclick = run;
    document.getElementById("setup").onclick = setup;
  }
});

export async function setup() {
  try {
    await Excel.run(async context => {
      
      //const range = context.workbook.getSelectedRange();
      var value1="";
      var value2="";
      const sheetName = "Sheet1";

      
      var QuestionCell = context.workbook.worksheets.getItem("Sheet1").getRange("A2:A4");
      var AnswerCell = context.workbook.worksheets.getItem("Sheet1").getRange("B2:B4");
      var Header = context.workbook.worksheets.getItem("Sheet1").getRange("A1:B1");  
      QuestionCell.values = [["Enter Patient Name"], ["Patient Weight"],[""]];
      AnswerCell.values= [[""], [""],[""]];
    
      Header.values = [["Questions","Your Answers"]];
    
      QuestionCell.format.autofitColumns();
      QuestionCell.format.autofitRows();
      AnswerCell.format.columnWidth = 150;
      AnswerCell.format.autofitRows();
      
      QuestionCell.format.fill.color =  "gray";
      AnswerCell.format.fill.color =  "yellow";
      Header.format.fill.color = "black";
      Header.format.font.color ="white";
    await context.sync();

    }).then(function () {
        console.log('Form Created for Getting Payload');
      
    });
  } catch (error) {
    console.error(error);
  }
}

export async function run() {
  try {
    await Excel.run(async context => {
      
      //const range = context.workbook.getSelectedRange();
      var value1="";
      var value2="";
      var value3="";

      var activeCell = context.workbook.worksheets.getItem("Sheet1").getRange("A1:B3");

      activeCell.load("values");
      await context.sync().then(function(){      
        console.log(activeCell.values[0][1]);
        value1 =   activeCell.values[0][1];
        value2 =    activeCell.values[1][1];
        value3= activeCell.values[2][1]
      });

      //
       //const Authorization = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6InBpVmxsb1FEU01LeGgxbTJ5Z3FHU1ZkZ0ZwQSIsImtpZCI6InBpVmxsb1FEU01LeGgxbTJ5Z3FHU1ZkZ0ZwQSJ9..x9a8RVNWsq7Z2JomY3SOyPJFV6rxnjJ-ATzHs-XJGn8aGks6CgFIY1vkOw2I-4Ii7bOuIyhzqRoFXNjtdJskct0HdE1qkmaKzHmTZelKiToudsRE5YQLc80gtFRfLfW5kJN_K7B7d3EM2V-_i4YLOFYaRMXKzlQuXIySS8TmhRJEPpqgco6rkV6oGWKTGDF_TjzezI_D8dDJEwqsLBpBU2tf_re4x7NDgx0s2pVy8jLMpsKp3w7DlwADlgDZubksybLqnhCcQvY6vk3bpUFgfqnCL8qPLPWmQ0EqHvMLmcWFOtmU1aopMoaotE_ZLOZWYKk1C5zPrUNduZj6Dtg7Kg";
      // const OcpApimSubscriptionKey = "c732351ad851400aa9fd46a76cf843ab";
      const url = "https://use2-impact-cdpgateway.azure-api.net/api/CheckList";
      const data = { fname: value1 , weight: value2, height: value3 };
    //
    var options = {
      method: 'GET',
      headers: {  
        'Content-Type': 'application/json;charset=UTF-8',
        'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6InBpVmxsb1FEU01LeGgxbTJ5Z3FHU1ZkZ0ZwQSIsImtpZCI6InBpVmxsb1FEU01LeGgxbTJ5Z3FHU1ZkZ0ZwQSJ9..x9a8RVNWsq7Z2JomY3SOyPJFV6rxnjJ-ATzHs-XJGn8aGks6CgFIY1vkOw2I-4Ii7bOuIyhzqRoFXNjtdJskct0HdE1qkmaKzHmTZelKiToudsRE5YQLc80gtFRfLfW5kJN_K7B7d3EM2V-_i4YLOFYaRMXKzlQuXIySS8TmhRJEPpqgco6rkV6oGWKTGDF_TjzezI_D8dDJEwqsLBpBU2tf_re4x7NDgx0s2pVy8jLMpsKp3w7DlwADlgDZubksybLqnhCcQvY6vk3bpUFgfqnCL8qPLPWmQ0EqHvMLmcWFOtmU1aopMoaotE_ZLOZWYKk1C5zPrUNduZj6Dtg7Kg',
        'Ocp-Apim-Subscription-Key': "c732351ad851400aa9fd46a76cf843ab"
      }
    }

    console.log(data);
    
    fetch(url,options)        
    .then(function (response) {
      console.log("testasda");
      console.log(response.json());
      return response.json();
    })
    .then(function (data) {
      console.log("test");
      const items = data;
      console.log(items);
    });



    
    });
  } catch (error) {
    console.error(error);
  }
}